    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        <div className="text-right">
          <p className={`text-xl font-bold ${(floatingPnl || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            {(floatingPnl || 0) >= 0 ? '+' : ''}¥{(floatingPnl || 0).toFixed(2)}
          </p>
          <p className={`text-sm font-medium ${(floatingPnlRate || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
      ...
      {isCurrent ? (
        <div className="grid grid-cols-2 gap-x-4 gap-y-1 pt-3 border-t border-slate-100">
          ...
          <div>
            ...
            <p className={`font-semibold ${(currentPos.daily_pnl || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
